...
<style>
    ...
    /* 1. ANIMASI TOMBOL (Naik Turun + Shadow) */
...
<body class="pb-40 lg:pb-32"> 
    ...
    <div x-data="{
        ...
        // DATA PRODUK
        productName: 'Paket Lengkap S.O.P HRD',
        productImage: 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&q=80&w=800&h=1000', 
        ...
        submitOrder() {
            ...
            window.location.href = '{{ route("pembayaran") }}?product=' + encodeURIComponent(this.productName) + '&total=' + this.grandTotal;
        },
            ...
            <div class="max-w-screen-xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
                <a href="{{ route('homepage') }}" class="flex items-center gap-3 cursor-pointer group">
                    <img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjRzyTdfjkBugSP3Ew_vmkaeMQKl0XnZVR83kFV0LtKJXC4gVF_WTGPS57iCampIjdlGU09l_Ct0hw_2Tx51GiHj5uWr6fTYqzJirf8qpAKhwW0AsM-pYcam74_l25KpFvShEYQdkJ-UnuJQsuiP7qa7Ek85k0MWaF0X0pHGmJZ2imL8IQK9ip5M9s2sW0/s16000/Templatenesia%20Logo.jpg" 
                         class="w-10 h-10 rounded-lg object-cover shadow-sm group-hover:scale-105 transition-transform" alt="Templatenesia Logo">
                    <div>
        ...
        <main class="w-full max-w-screen-xl mx-auto px-4 sm:px-6 pt-24 lg:pt-28">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative">
                ...
                <div class="lg:col-span-7 space-y-10 animate-fade-in-up delay-100 pb-10">
                    ...
                    <div x-data="{ 
                            slides: [
                                { type: 'image', src: 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&q=80&w=800&h=1000' },
                                { type: 'video', id: 'tgbNymZ7vqY' }, 
                                { type: 'image', src: 'https://images.unsplash.com/photo-1554224154-26032ffc0d07?auto=format&fit=crop&q=80&w=1000&h=1000' }
                            ],
                            active: 0,
                            autoplay: null,
                         ...
                         class="bg-white p-3 rounded-[2rem] shadow-soft border border-slate-100 relative group overflow-hidden w-full">
                             ...
                             <template x-for="(slide, index) in slides" :key="index">
                                <div x-show="active === index" 
                                    ...
                                    <template x-if="slide.type === 'video'">
                                        <iframe class="w-full h-full absolute inset-0 rounded-lg" :src="'https://www.youtube.com/embed/' + slide.id" frameborder="0" allowfullscreen></iframe>
                                    </template>
                                </div>
                            </template>
                    ...
                    <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-soft">
                        ...
                        <div class="prose prose-slate max-w-none text-slate-500 leading-relaxed mb-6">
                            <p>Produk ini didesain khusus untuk membantu Anda mempercepat proses bisnis tanpa ribet teknis. Menggunakan standar <span class="bg-blue-50 text-iosBlue px-1 rounded font-bold">iOS Clean Design</span>.</p>
                        </div>
                        ...
                        <div class="bg-blue-50/50 p-6 rounded-2xl border border-blue-100/50">
                            <h4 class="font-bold text-slate-900 mb-4 text-sm uppercase tracking-wide">Manfaat untuk Bisnis Anda:</h4>
                            <ul class="space-y-3">
                                <li class="flex items-start gap-3"><i class="ri-checkbox-circle-fill text-green-500 text-xl mt-0.5"></i><span class="text-slate-700 font-medium">Meningkatkan Omset & Profitabilitas</span></li>
                                <li class="flex items-start gap-3"><i class="ri-checkbox-circle-fill text-green-500 text-xl mt-0.5"></i><span class="text-slate-700 font-medium">Membuat Performa Kinerja Tim Lebih Terstruktur & Mandiri</span></li>
                                <li class="flex items-start gap-3"><i class="ri-checkbox-circle-fill text-green-500 text-xl mt-0.5"></i><span class="text-slate-700 font-medium">Bisnis Terlihat Lebih Professional & Siap Skalasi</span></li>
                                <li class="flex items-start gap-3"><i class="ri-checkbox-circle-fill text-green-500 text-xl mt-0.5"></i><span class="text-slate-700 font-medium">Meningkatkan Efisiensi Waktu (Owner tidak perlu mengawasi 24 jam)</span></li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div x-data="{
                            testimonials: [
                                { name: 'Raka Ardhi', role: 'Freelancer', text: 'Gila sih, materinya daging semua. Langsung saya praktekkan dan closing klien pertama dalam 3 hari!', initial: 'RA' },
                                { name: 'Diana Puspita', role: 'Olshop Owner', text: 'Tampilan ebooknya enak dibaca, dan bonus template invoice-nya kepake banget buat bisnis saya.', initial: 'DP' },
                                { name: 'Bayu Saputra', role: 'Entrepreneur', text: 'Awalnya ragu, tapi supportnya juara. Ada kendala dikit langsung dibantu tim support sampai beres.', initial: 'BS' },
                                { name: 'Sarah Wijaya', role: 'HRD Manager', text: 'Sangat membantu merapikan SOP di kantor kami. Formatnya mudah diedit dan sangat professional.', initial: 'SW' },
                                { name: 'Reza Pratama', role: 'Content Creator', text: 'Investasi terbaik bulan ini. Bonusnya saja sudah seharga produknya. Recommended banget!', initial: 'RP' }
                            ],
                            current: 0,
                            autoplay: null,
                            init() { this.startAutoplay(); },
                    ...
                    <div x-data="{ active: 0, faqs: [ 
                        { q: 'Apakah file bisa diedit?', a: 'Ya, 100% editable. Anda akan mendapatkan format Microsoft Word, Excel, dan PDF yang tidak dikunci.' },
                        { q: 'Bagaimana cara akses produknya?', a: 'Link download akan dikirim otomatis ke WhatsApp dan Email Anda detik itu juga setelah pembayaran berhasil.' },
                        { q: 'Apakah pembayaran sekali bayar?', a: 'Betul! Cukup bayar sekali saja (One-time payment) untuk akses seumur hidup.' },
                        { q: 'Apakah ada garansi uang kembali?', a: 'Tentu. Jika file rusak atau tidak sesuai deskripsi, kami kembalikan uang Anda 100%.' }
                    ] }" class="bg-white rounded-[2rem] border border-slate-100 shadow-soft overflow-hidden">
                    ...
                    <div class="bg-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden shadow-2xl">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="space-y-3 mb-8">
                                <template x-for="item in ['E-Book Masterclass', 'Video Tutorial HD', 'Template Kalender', 'Cheatsheet Copy', 'Akses Private Group']">
                                    <div class="bg-white/5 p-3 rounded-xl flex justify-between items-center border border-white/5 hover:bg-white/10 transition-colors">
                                        <div class="flex items-center gap-3"><i class="ri-checkbox-circle-fill text-green-400 text-lg"></i><span class="font-medium text-slate-200 text-sm" x-text="item"></span></div>
                            ...
                            <div class="bg-gradient-to-r from-iosBlue to-iosPurple rounded-xl p-5 text-center shadow-lg">
                                ...
                                <div class="flex items-center justify-center gap-3"><span class="text-white/50 line-through text-sm">Rp 1.298.000</span><span class="text-2xl font-extrabold text-white">GRATIS HARI INI</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="pt-2 border-t border-slate-100">
                                <div id="agree-section" class="mb-4">
                                    <label class="flex items-start gap-2 cursor-pointer">
                                        <input type="checkbox" id="agree" x-model="formData.agreed" class="mt-0.5 w-4 h-4 text-iosBlue rounded border-slate-300 focus:ring-iosBlue" :class="{'border-red-500': formErrors.agreed}">
